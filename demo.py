...
children=[
            ...
            children=[
                ...
                children=[
                    ...
                    children=[
                        ...
                        html.A(className="text-black-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-base font-medium",children="Paper", href="https://drive.google.com/file/d/1-6kqS-NCtMVsmeVPcyF6oKc8CC6Vtqsp/view?usp=sharing"),
                        html.A(className="text-black-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-base font-medium",children="About me", href="https://github.com/RC-Sho0"),
                    ])
                ])
            ])
        ])
    ...
    html.Div(id='main',children=[
        html.Div(children=[
            html.Div(children=[
                dcc.Graph(id='upload', figure= blank_fig()),
            ], className="items-center flex-1"),
            html.Div(children=[
                html.Ol("Cách sử dụng!!!",className="font-semibold text-xl list-decimal list-inside"),
                html.Li("Kéo và thả hoặc chọn hình ảnh của bạn."),
                html.Li(children=[
                    html.Li("Chọn tỉ lệ:    ",),
                    dcc.Dropdown(['2','4'],'2', id='scale', className="w-20", clearable=False),
                ], className="flex items-center justify-center"), 
                html.Li("Chọn vùng cần nâng cấp độ phân giải."),
                html.Li("Nhấn Predict và xem kết quả!."),
            ],className="flex-1 flex flex-col items-center justify-between h-80 pt-6"),
...
@app.callback(
    ...
    Output('SRCNN', 'figure'),
    Output('EDSR', 'figure'),
    [State('upload', 'relayoutData')],
    [State('scale', 'value')],
    [Input('predict', 'n_clicks')])
def display_relayout_data(relayoutData, scale, n_clicks):
    # print(f"Print : {json.dumps(relayoutData)}")
    global scl
    global SRCNN
    global EDSR
    global BICUBIC
    ...
    if 'xaxis.range[0]' in json.dumps(relayoutData):
        ...
        return draw_plot(img,"LR"), draw_plot(bicubic_img, "BICUBIC") ,draw_plot(srcnn_img, "SRCNN"), draw_plot(edsr_img, "EDSR")
    elif 'xaxis.autorange' in json.dumps(relayoutData):
        img = hr_img
        # print("HR--",hr)
        if scl ==  None  or  scl  != scale:
            scl = scale 
